...
- [ ] Add results-updated banner: when admin syncs ESPN results, track the timestamp in the tournaments table. Show a dismissible banner on the dashboard telling users "Results updated at [time]" so friends know the leaderboard is fresh. Dismissal persists via localStorage until the next update. Inspired by MarchMadness app's ResultsBanner.

...
- [ ] Add scoring breakdown dialog: clicking a user's score on the leaderboard opens a dialog showing every resolved pick — round, team picked, correct/wrong, base points, upset bonus. Helps friends see exactly which picks earned or lost points. Reuse existing scorePicks logic, extend it to return per-game detail.
